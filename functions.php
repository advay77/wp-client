...
define( 'ADVAY_THEME_VERSION', '2.48.0' );
...
function advay_intake_email_url() {
	...
	if ( ! is_string( $value ) || '' === trim( $value ) ) {
		$value = 'mailto:info@eliteprepcenter.com';
	}
...
function advay_dock_email_label() {
	...
	if ( ! is_string( $value ) || '' === trim( $value ) ) {
		$value = 'info@eliteprepcenter.com';
	}
...
/**
 * WhatsApp — primary text channel for seller inquiries.
 *
 * @return string
 */
function advay_whatsapp_url() {
	...
	if ( ! is_string( $value ) || '' === trim( $value ) ) {
		$value = 'https://wa.me/12128148815';
	}
	return apply_filters( 'advay_contact_whatsapp_url', $value );
}

/**
 * Floating dock — WhatsApp CTA label.
 *
 * @return string
 */
function advay_dock_whatsapp_label() {
	return apply_filters(
		'advay_dock_whatsapp_label',
		__( 'Message us on WhatsApp', 'advay-theme' )
	);
}
